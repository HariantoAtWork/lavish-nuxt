...
// import list from '@@/example/example.json'
import { createEventList } from '@factories/events'
...
export default {
  ...
  computed: {
    events() {
      const events = createEventList(/*{ list: this.list }*/)
      events.setData({
        name: 'Events',
        list: this.list
      })
      const { state, methods, components } = events
  ...
  watch: {
    list(newValue, originalValue) {
      console.log('WATCH', { newValue, originalValue })
      this.clearSelectedEvent()
    }
  },
  methods: {
    clearSelectedEvent() {
      const originalUuid = this.event.uuid
      const found = this.list.find(item => item.uuid === originalUuid)
      this.event = found ? found : {}
      if (!this.list.length) this.settings.editMode = true
    },
    addEvent() {
      const {
        methods,
      ...
      console.log({ methods })
      this.event = methods.addItem({ name })
      formData.eventName = ''
    },
  ...
  <aside class="aside">
    <ul class="mb-4">
      <li
        ...
        :key="item.uuid"
        :tabindex="index + 1"
        @keypress.enter="onSelectEvent(item)"
        @keyup.delete="onDeleteEvent(item)"
        :class="{ active: event.uuid === item.uuid }"
      >
        <div
        ...
        <i
          ...
          class="fa-duotone fa-trash p-2"
        ></i>
      </li>
    </ul>
    <div class="field p-4" v-if="settings.editMode">
      <v-text-field
        v-model="formData.eventName"
        @keypress.enter="addEvent"
        :label="$__('Events.addEvent', 'Add Event')"
        hide-details
      >
        <template v-slot:append>
          <v-btn @click="addEvent"><v-icon> mdi-plus </v-icon></v-btn>
        </template>
      </v-text-field>
    </div>
  </aside>
