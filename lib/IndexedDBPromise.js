...
const IndexedDBPromise = function (databaseName, tableName, options) {
  ...
  const openTransaction = (mode, { tableName, db }) =>
    new Promise((resolve, reject) => {
      ...
      tx.onerror = error.bind(console, 'tx: onerror', tableName)
    })
  ...
  const getDataFromStoreByItem = ({ store, item }) =>
    new Promise((resolve, reject) => {
  ...
  const get = (id, { store, tx, db }) =>
    new Promise((resolve, reject) => {
      if (Array.isArray(id)) {
        ...
        console.log(Object.entries(id)[0])
        tx.onsuccess = log.bind(console, 'get Object')
        resolve(
          getDataFromStoreByItem({ store, item: id }).then(result => {
            db.close()
            return result
          })
        )
  ...
  const methods = {
    put(data) {
      return readwriteTransaction(databaseName, tableName).then(
        put.bind(put, data)
      )
    },
    remove(id) {
      return readwriteTransaction(databaseName, tableName).then(
        remove.bind(remove, id)
      )
    },
    get(id) {
      return readonlyTransaction(databaseName, tableName).then(
        get.bind(get, id)
      )
    },
    getAll() {
